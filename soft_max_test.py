  ...
  def test_grad_norm_inequalities(s):
    for n in range(2, 100):
      for i in range(1000):
        ...
        s.assertLessEqual(la.norm(grad_x, 1), 1 + 1e-8)
        s.assertGreaterEqual(grad_x.dot(x), lmax_x - math.log(2*n))

        y = np.random.normal(scale=n, size=(n))
        lmax_y = soft_max(y)
        grad_y = grad_soft_max(y)
        s.assertLessEqual(la.norm(grad_x - grad_y, 1), la.norm(x - y, np.inf))
